...
TEST(JoinTests, JoinDegOneTrueWithCompress){
    const auto pp = Join::pp_gen(1, 10);
    const auto msk = Join::msk_gen(pp, {}, true);
...
TEST(JoinTests, JoinDegMulSelTrueWithCompress){
    const auto pp = Join::pp_gen(5, 10);
    const auto msk = Join::msk_gen(pp, {}, true);
...
TEST(JoinTests, JoinDifferentLengthTrueWithCompress){
    // Generate two set of pp and msk.
    const auto pp1 = Join::pp_gen(1, 10);
    const auto msk1 = Join::msk_gen(pp1, {}, true);
    const auto pp2 = Join::pp_gen(1, 8);
    auto msk2 = Join::msk_gen(pp2, msk1.hmac->get_key(), true);
